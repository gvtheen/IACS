...
#include "CIOGjf.h"
#include "CIOPoscar.h"
#include "CCalcClusterStructPool.h"
...
void CCalcClusterStructPool::init()
{
...
void CCalcClusterStructPool::Initialization(const std::vector<std::string*>& inputfiles)  // initialize from exit
{
   std::string str;
   std::vector<std::string> vecStr;
   Cios* inputIO;
...
size_t CCalcClusterStructPool::ClusterType(std::vector<CATAZJUT::CElement*>& mht)
{
...
void CCalcClusterStructPool::mixedClusterPredict(CATAZJUT::CPeriodicFramework* predict_struct)
{
    //GACatalysis(nousing)
}
void CCalcClusterStructPool::eliminateCloseContacts(CATAZJUT::CPeriodicFramework* curr_struct,double distanceCutOff)
{
    util::Vector3 vect;
